  ...
  async findById(id: string): Promise<Pokemon | null> {
    ...
    try {
        if (isNumeric) {
            ...
            pokemon = await this.firestore.whereEqualTo('name', id.toLowerCase()).findOne();
        }
    } catch (e) {
        this.logger.warn(`Firestore unavailable or not configured, skipping cache check. Error: ${e.message}`);
    }
    ...
    try {
        const { data } = await firstValueFrom(this.http.get(`https://pokeapi.co/api/v2/pokemon/${id.toLowerCase()}`));
